<?php
    ...
    session_start();
    $id = $_SESSION["id"];
    $id_contributor_md = $_SESSION['counted_id_md'];
    $product_id_array_md = $_SESSION['product_id_array_session_md'];
    $id_contributor_md_last_id = $_SESSION['counted_id_md_last_id'];
    ...
    if(isset($_POST['submit'])){
        ...
        // send data to bd
        $id_contributor_md_last_id_intified = (int)$id_contributor_md_last_id;
        $last_contributor_id = ($id_contributor_md_last_id - $id_contributor_md) + 1;
        for($i = 0; $i <= $id_contributor_md_last_id; $i++){
            if(!empty($_POST['product_id_'.$i.''])){
                $brif_description_array_md[$i] = $_POST['product_id_'.$i.''];
            }
        }
        for($i = 1; $i <= $id_contributor_md; $i++){
            $brif_description_array_field_content[$i] = $product_id_array_md[$i];
        }
        for($i = 1; $i <= $id_contributor_md; $i++){
            $brif_description_array_field_content_last_id[$i] = $product_id_array_md[$i];
        }
        if(array_filter($errors)){
            ...
            for($i = 0; $i <= $id_contributor_md_last_id_intified; $i++){
                $brif_sql = "INSERT INTO fields_content(product_id, content, field_id)
                VALUES('$final_id_connector', '$brif_description_array_md[$i]', '$brif_description_array_field_content_last_id[$i]')";
                if($brif_description_array_md[$i] != ''){
...
<html lang="en">
    ...
    <div class="user_crud_container">
        <h1 class="user_crud_container_intor_text">Add <strong class="brake_point_strong_md">Mobile Device</strong> Proudct Page</h1>
                        ...
                        if($products_brif_result){
                            while($products_brif_row = mysqli_fetch_assoc($products_brif_result)){
                                ...
                                if($product_type == 'MD'){
                                    ...
                                    ';
                                    $id_contributor_md++;
                                    $last_product_id = $id_contributor_md;
                                    $last_product_id_solid_outer = $product_id;
                                }
                                $last_product_id_outer = $last_product_id_solid_outer;
                            }
                            $test = $last_product_id_outer - $id_contributor_md;
                            $last_product_id_outer_simplifyed = $last_product_id_outer;
                            for($i = $last_product_id_outer; $i != $test; $i--){
                                $product_id_array_md[$i] = $last_product_id_outer--;
                            }
                        }
                        $last_product_id_outer_intified = (int)$last_product_id_outer_simplifyed;
                        $_SESSION['counted_id_md'] = $id_contributor_md;
                        $_SESSION['counted_id_md_last_id'] = $last_product_id_outer_intified;
                        $_SESSION['product_id_array_session_md'] = $product_id_array_md;
                    ?>                                   
                </div>
                </div>
            </section>  
            <input type="text" class="add_product_hidden" name="product_type" required value="Mobile device">
