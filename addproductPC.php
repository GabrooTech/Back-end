<?php
    ...
    $id_contributor = $_SESSION['counted_id'];
    $product_id_array = $_SESSION['product_id_array_session'];
    ...
    = $third_img = $fourth_img = $fifth_img = $description = $product_type = $subject = $subject_description = '';
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($_POST['main_photo']) && $_POST['main_photo'] != ' '){
        ...
        for($i = 0; $i <= $id_contributor; $i++){
            if(!empty($_POST['product_id_'.$i.''])){
                $brif_description_array[$i] = $_POST['product_id_'.$i.''];
            }
        }
        for($i = 1; $i <= $id_contributor; $i++){
            $brif_description_array_field_content[$i] = $product_id_array[$i];
        }
        print_r($brif_description_array_field_content);
        echo $id_contributor;
        // echo gettype($id_contributor);
        ...
        if(array_filter($errors)){
            ...
            //     VALUES('$final_id_connector', '$brif_description_array[$i]', '$brif_description_array_field_content[$i]')";
            //     if($brif_description_array[$i] != ''){
...
<html lang="en">
    ...
    <div class="user_crud_container">
        ...
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST" class="add_product_form_main_class" id="main_form">
                        ...
                        if($products_brif_result){
                            while($products_brif_row = mysqli_fetch_assoc($products_brif_result)){
                                ...
                                if($product_type == 'PC'){
                                    ...
                                    ';
                                    $id_contributor++;
                                    $last_product_id = $id_contributor;
                                    // echo $last_product_id;
                                }
                            }

                            $test = $last_product_id - $id_contributor;

                            for($i = $last_product_id; $i != $test; $i--){
                                $product_id_array[$i] = $last_product_id--;
                            }
                        }
                        $_SESSION['counted_id'] = $id_contributor;
                        $_SESSION['product_id_array_session'] = $product_id_array;
                    ?>                                   
                </div>
                </div>
            </section>                            
            <input type="text" class="add_product_hidden" name="product_type" required value="PC">
    ...
    include('footer.php'); 
    ?>
    <!-- <script src="addReasonContent.js"></script> -->
