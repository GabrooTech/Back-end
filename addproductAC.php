<?php
    ...
    session_start();
    $id = $_SESSION["id"];
    $id_contributor_accessories = $_SESSION['counted_id_accessories'];
    $product_id_array_accessories = $_SESSION['product_id_array_session_acceossories'];
    $id_contributor_accessories_last_id = $_SESSION['counted_id_accessorie_last_id'];
    ...
    if(isset($_POST['submit'])){
        ...
        $id_contributor_server_last_id_intified = (int)$id_contributor_accessories_last_id;
        $last_contributor_id = ($id_contributor_accessories_last_id - $id_contributor_accessories) + 1;
        for($i = 0; $i <= $id_contributor_accessories_last_id; $i++){
            if(!empty($_POST['product_id_'.$i.''])){
                $brif_description_array_accessories[$i] = $_POST['product_id_'.$i.''];
            }
        }
        for($i = 1; $i <= $id_contributor_accessories; $i++){
            $brif_description_array_field_content[$i] = $product_id_array_accessories[$i];
        }
        for($i = 1; $i <= $id_contributor_accessories_last_id; $i++){
            $brif_description_array_field_content_last_id[$i] = $product_id_array_accessories[$i];
        }
        ...
        if(array_filter($errors)){
            ...
            for($i = 0; $i <= $id_contributor_server_last_id_intified; $i++){
                ...
                VALUES('$final_id_connector', '$brif_description_array_accessories[$i]', '$brif_description_array_field_content_last_id[$i]')";
                if($brif_description_array_accessories[$i] != ''){
...
<html lang="en">
    ...
    <div class="user_crud_container">
        <h1 class="user_crud_container_intor_text">Add <strong class="brake_point_strong_accessories">Accessories</strong> Proudct Page</h1>
                        ...
                        if($products_brif_result){
                            while($products_brif_row = mysqli_fetch_assoc($products_brif_result)){
                                ...
                                if($product_type == 'Accessories'){
                                    ...
                                    ';
                                    $id_contributor_accessories++;
                                    $last_product_id = $id_contributor_accessories;
                                    $last_product_id_solid_outer = $product_id;
                                }
                                $last_product_id_outer = $last_product_id_solid_outer;
                            }
                            $test = $last_product_id_outer - $id_contributor_accessories;
                            $last_product_id_outer_simplifyed = $last_product_id_outer;
                            for($i = $last_product_id_outer; $i != $test; $i--){
                                $product_id_array_accessories[$i] = $last_product_id_outer--;
                            }
                        }
                        $last_product_id_outer_intified = (int)$last_product_id_outer_simplifyed;
                        $_SESSION['counted_id_accessories'] = $id_contributor_accessories;
                        $_SESSION['counted_id_accessorie_last_id'] = $last_product_id_outer_intified;
                        $_SESSION['product_id_array_session_acceossories'] = $product_id_array_accessories;
                    ?>                                   
                </div>
                </div>
            </section>   
            <input type="text" class="add_product_hidden" name="product_type" required value="Accessories">
